...
for i in range(100):
    ...
    for j in range(681):
        ...
        if 1 <= lidar_distance < 4:
            object_x += np.random.normal(0, 0.1)
            object_y += np.random.normal(0, 0.1)
    ...
    plt.scatter(object_coordinates[:, 0], object_coordinates[:, 1], color='red', s=2)
plt.plot(robot_x, robot_y, color='green' if i == 0 else None)
